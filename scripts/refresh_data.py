...
- Do NOT include <cite> tags or any other citation markup anywhere in the output.
  Put source attribution in the "latest_source" field as plain text instead.
- Keep each "note" field to 2-3 sentences so the full response fits comfortably
  within the token budget.
- Ensure the JSON is complete and valid, with every object and array closed."""
...
def call_model(client, today_str):
    # max_tokens must comfortably fit 13 member entries plus any thinking
    # tokens, which count against the same budget. 16k gives ample headroom.
...
def strip_citations(text: str) -> str:
    """Remove <cite index="...">...</cite> wrappers, keeping the inner text.

    The web search tool encourages the model to emit citation tags. Inside
    JSON string values these add escaped quotes and burn output tokens, so
    we unwrap them while preserving the wording.
    """
...
def main() -> int:
    ...
    if getattr(response, "stop_reason", None) == "max_tokens":
        print(
            "ERROR: The model hit the max_tokens limit and the JSON was cut off "
            "mid-response. Raise max_tokens in call_model() and re-run.",
            file=sys.stderr,
        )
        return 1
